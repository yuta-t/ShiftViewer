...
struct Term {
    ...
    init(id: Int, groupId: Int, start: String, finish: String) {
        self.id = id
        self.group = Group.init(rawValue: groupId)!
        startDate = NSDate.convert(start)
        finishDate = NSDate.convert(finish)
    }
}